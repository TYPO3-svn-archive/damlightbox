...
final class tx_damlightbox_div {
	...
	/**
	 * Dynamically adds the universal pseudo fields to the current TCA. Only called from TCEForms if the incoming tablename has been tested as legitimate.
	 *
	 * @param	string		$table: tablename
	 * @return	boolean		TRUE if it worked, FALSE if it didn't
	 */	
	static function addDamlightboxFieldsToTableTCA($table) {
		
		// check the tables for which the fields are allowed
		$allowedTables = t3lib_div::trimExplode(';', $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['damlightbox']['allowedTables'], 1);

		if (is_array($allowedTables)) {
			
			// walking through the configuration string
			foreach ($allowedTables as $configstring) {
				
				$tableconfig = t3lib_div::trimExplode('|', $configstring, 1);
				
				if ($tableconfig[0] == $table) {
					
					// prepare the flexform field
					$tempColumns = Array (
					...
					// test for the different options for the addToAllTCAtypes function (which types, after which fields, use any other reference fields etc.)
					foreach ($tableconfig as $config) {
						...
						// if another reference field (like dam_ttcontent) is specified, remove the pseudo image field
						if (strpos($config, 'reffield:') !== FALSE) {
							
							$fields = 'tx_damlightbox_flex';
							unset($tempColumns['tx_damlightbox_image']);
							
						// otherwise add both fields
						} else {
					...
					// finally call the according functions for adding the fields
